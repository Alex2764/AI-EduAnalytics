...
Генерира Word документ със статистика и **проценти по въпроси** (`q1_success` …) директно от резултатите в базата — **без извикване на Gemini** (не хаби API токени).

Текстовите AI секции в документа се взимат от **кеш** (`test_analytics.ai_analysis`), ако вече сте натиснали „Генерирай AI анализ“. Иначе в документа има кратко напъствие да генерирате AI отделно.

За нов AI текст: `POST /api/analytics/{test_id}/generate-analysis`

**Забележка:** Използва се последно каченият шаблон. Качи нов шаблон през AI Settings → `/api/templates/upload`.
